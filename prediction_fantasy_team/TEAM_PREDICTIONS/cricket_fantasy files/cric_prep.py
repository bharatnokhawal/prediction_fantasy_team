...
def preprocess_data(data):
    required_columns = ['ball_faced', 'run_scored', 'ball_delivered', 'run_given', 'wicket', 'against_team', 'player']
...
def calculate_fantasy_points(player_stats):
    ...
    return points
# Calculate fantasy points for a player based on their performance stats
def calculate_fantasy_points_players(player_stats):
    points = 0
...
# Calculate a player's historical performance against a specific bowler
def performance_against_player(batsman, bowler, df):
...
def main():
    ...
    trans_data = 'IPl Ball-by-Ball 2008-2023.csv'
    data = load_data(transformed_file)
    cric_data = load_data(trans_data)
    ...
    teams = [['V Kohli' , 'GJ Maxwell' ,  'Mohammed Siraj',  'C Green' ,'RM Patidar' , 'MK Lomror' , 'Yash Dayal',  'KD Karthik' ,'KV Sharma' , 'F du Plessis' ,  'LH Ferguson' ], ['SV Samson', 'TK Cadmore', 'YBK Jaiswal', 'TA Boult', 'R Parag', 'SO Hetmyer', 'R Ashwin', 'Dhruv Jurel', 'YS Chahal', 'R Powell', 'Sandeep Sharma']]
    against_teams = ['Rajasthan Royals', 'Royal Challengers Bangalore']
    all_impact_scores = {}
    ...
    for team in teams:
        for player_name in team:
            player_stats = performance_against_team(player_name, against_team, data)
            fantasy_points[player_name] = calculate_fantasy_points(player_stats)
            for bowler in team:
                if player_name != bowler:
                    player_bowler_stats = performance_against_player(player_name, bowler, cric_data)
                    fantasy_points[player_name] += calculate_fantasy_points_players(player_bowler_stats)
